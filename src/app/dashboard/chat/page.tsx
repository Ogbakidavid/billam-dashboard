...
export default function ChatSimulatorPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (updated.state === 'FAILED_RETRY' || updated.error_message) {
        console.error('sendMessage response:', updated);
      }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[900px] mx-auto space-y-4">
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[13px] text-[#6F716E]">Experience BillAm from the client&apos;s side — responses come directly from the AI agent</p>
          </div>
              ...
              {selectorOpen && (
                ...
                >
                  ...
                  {CLIENT_PERSONAS.map(persona => (
                    <button
                      key={persona.id}
                      onClick={() => { setSelectedPersonaId(persona.id); setSelectorOpen(false); }}
      ...
      >
        ...
        <div className="w-px h-8 bg-[#E7E7E3] hidden sm:block" />
        <div>
          <p className="text-[10px] font-semibold text-[#999C98] uppercase tracking-wider">Business</p>
        ...
        {jobId && (
          <>
            <div className="w-px h-8 bg-[#E7E7E3] hidden sm:block" />
            <div>
              <p className="text-[10px] font-semibold text-[#999C98] uppercase tracking-wider">Job ID</p>
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#E7E7E3] bg-[#FAFAF9] shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            <span className={`w-2 h-2 rounded-full ${jobId ? 'bg-[#19D66B] animate-pulse' : 'bg-[#999C98]'}`} />
            <span className={`text-[11px] font-semibold ${jobId ? 'text-[#19D66B]' : 'text-[#999C98]'}`}>
              {jobId ? 'Active' : 'Idle'}
            </span>
          </div>
        </div>
        ...
        <div className="px-3 py-3 border-t border-[#E7E7E3] bg-white shrink-0">
          ...
          <p className="text-[11px] text-[#999C98] mt-1.5 px-1">
            ...
            {jobId && <span className="ml-1">· Job <span className="font-mono text-[#079A4F]">{jobId.slice(0, 8)}…</span></span>}
          </p>
        </div>
      </div>
    </div>
