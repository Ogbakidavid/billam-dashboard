...
export default function ClientsPage() {
  ...
  // Derive clients from allJobs
  const clientMap = new Map<string, any>();
  currentPersona.allJobs.forEach(job => {
    const key = job.phone || job.client;
    if (!clientMap.has(key)) {
      clientMap.set(key, {
        ...
        activeJobs: 0,
        prevJobs: 0,
        lastInteraction: job.lastActivity,
        latestQuote: `₦${job.amount.toLocaleString()}`,
        status: job.state,
      });
    }
    const c = clientMap.get(key);
    if (['EXECUTED', 'CANCELLED', 'FAILED_RETRY'].includes(job.state)) {
      c.prevJobs++;
    } else {
      c.activeJobs++;
    }
  ...
  const stateToStatusMap: Record<string, StatusType> = {
  ...
  const clients = Array.from(clientMap.values()).map(c => ({
    ...c,
    status: stateToStatusMap[c.status] || 'REASONING'
  }));
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 max-w-350">
      {/* Header */}
